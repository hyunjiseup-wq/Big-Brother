...
- **모델이 다르면 판단도 달라질 수 있음**: Gemini/Groq/로컬 모델은 학습 데이터와 안전 기준이 달라
  ...
  "판단 주체"에 `Ollama(3차 폴백·로컬)`로 표시되므로 폴백 판단만 따로 검증할 수 있습니다.
...
- `!BB 검토대기`의 "판단 주체"와 `!BB 상태`의 "판단 폴백 사슬"로 폴백이 얼마나 자주 발동하는지
  파악하세요. `Ollama(3차 폴백·로컬)` 판단이 잦다면 클라우드 무료 한도가 상시 마르고 있다는 뜻이므로,
  금칙어 목록 보강·`MIN_LENGTH_FOR_AI_CHECK` 상향으로 호출을 줄이거나 Gemini 한도를 유료로
  올리는 것을 고려해보세요.
...
- 오탐(false positive) 방지를 위해 Gemini/Groq/Ollama가 **모두** 실패하면 안전하게 `NONE`(위반 없음)으로
  처리합니다. 이 상태가 `AI_OUTAGE_ALERT_THRESHOLD`회 연속되면 서버별로 장애 경고가 로그 채널에
  올라오며, 경고에는 3차 폴백까지 포함한 점검 항목이 함께 표시됩니다.

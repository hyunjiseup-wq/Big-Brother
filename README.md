...
5. **킥/밴은 절대 자동 실행되지 않습니다.** 실제 커뮤니티 정책(경고 2회 → 운영진이 최종 판단,
   ...
   하향되고, 로그 채널에 "⚠️ 관리자 검토 필요"로 강조 표시됩니다. `!automod 검토대기` 명령어로
...
## 명령어 (서버 내에서 `!automod ` 접두사 사용)

| 명령어 | 권한 | 설명 |
|---|---|---|
| `!automod 점수 @유저` | 메시지 관리 권한 | 해당 유저의 누적 위반 점수 및 최근 이력(판단 모델 포함) 확인 |
| `!automod 점수초기화 @유저` | 관리자 권한 | 유저의 누적 점수 초기화 |
| `!automod 규칙확인` | 누구나 | 현재 설정된 서버 규칙 확인 |
| `!automod 검토대기 [시간]` | 메시지 관리 권한 | 킥/밴이 자동 실행 대신 하향 조정된 건 목록 (기본 최근 72시간) |
| `!automod 오탐학습 [개수]` | 메시지 관리 권한 | 활성 오탐 학습 규칙과 적용 범위 조회 |
| `!automod 오탐취소 <규칙번호>` | 관리자 권한 | 잘못 등록한 오탐 학습 규칙 비활성화 |
| `!automod 상태` | 메시지 관리 권한 | 처리 대기열/워커/드롭 건수 확인 |

검수 카드의 `정상 · 이 채널 학습`은 해당 채널에서만 동일 오탐을 허용합니다. 스레드에서는
부모 채널 범위로 적용됩니다. `정상 · 서버 전체 학습`은 관리자만 사용할 수 있으며 모든 채널에
적용되므로, 어디서나 정상인 문구가 확실할 때만 사용하세요. 기존 `false_positive` 검수 이력은
...
- `!automod 검토대기`를 주기적으로 확인해 폴백이 얼마나 자주 발동하는지 파악하고,
...
필요하면 `!automod 감사실행 [gemini|groq|ollama|auto]` 명령어로 즉시 수동 실행도 가능합니다
(관리자 권한 필요).
...
| 명령어 | 권한 | 설명 |
|---|---|---|
| `!automod 감사실행 [backend]` | 관리자 권한 | 배치 감사를 지금 바로 실행 (backend 생략 시 `config.BATCH_BACKEND` 사용) |

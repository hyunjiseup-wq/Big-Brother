...
서버 규칙을 기준으로 AI(Gemini 1차 / Groq 2차 / 로컬 Ollama 3차)가 메시지를 판단하여
경고 → 삭제 → 타임아웃 → 킥 → 밴까지 단계적으로 자동 제재하는 discord.py 기반 봇입니다.
무료 API 티어로 운영 가능하도록 설계했으며, 무료 한도가 모두 소진돼도 한도가 없는 로컬
모델이 마지막 폴백으로 감시를 이어갑니다.
...
├── moderator.py     # Gemini(1차)→Groq(2차)→Ollama(3차)로 메시지 위반 여부 판단, 배치 판단 지원
...
2. 애매한 메시지만 `moderator.py`로 넘어가 **Gemini**에게 먼저 판단을 요청합니다.
   Gemini가 실패하거나(오류/한도초과) 응답을 못 주면 자동으로 **Groq**로, Groq까지 막히면
   호출 한도가 없는 **로컬 Ollama**로 폴백합니다 (`config.OLLAMA_REALTIME_FALLBACK`).
...
- **왜 삼중화하나**: Gemini도 Groq도 **무료 티어라 각자 일일 한도가 있습니다.** 한쪽만 쓰면 트래픽이
  ...
  **사실상 키워드 필터만 남은 무감시 상태**가 됩니다. 3차 폴백인 로컬 Ollama는 호출 한도가 없어
  이 구멍을 막는 마지막 그물입니다.
- **3차 폴백 켜는 법**: `config.OLLAMA_REALTIME_FALLBACK = True`(기본값)로 두고, 봇이 도는 PC에
